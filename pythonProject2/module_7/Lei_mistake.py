...
def get_station_info(station_code: str):
    ...
    if response.status_code == 200:
        ...
        if latitude and longitude:
            print(f"Station: {station_name}, Coordinates: ({latitude}, {longitude})")
...
def main():
    ...
    # Collect lon/lat pairs for each station
    incident_locations = []
    ...
    # Limit to 20 stations for map rendering
    incident_locations = incident_locations[:20]

    # Convert the list of lon/lat pairs to a URL-encoded GeoJSON blob
    encoded_geo_json = encode_geojson(incident_locations)

    # Retrieve and download the static map image from MapBox API
